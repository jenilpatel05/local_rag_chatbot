...
with st.sidebar:
    ...
    st.caption("Powered by Llama 3, ChromaDB and Ollama")
    st.divider()
    ...
    if uploaded_files:
        for uploaded in uploaded_files:
            ...
            if chunks > 0:
                ...
                st.info(f"{uploaded.name} already ingested (0 new chunks)")
        st.session_state.ingested = list_ingested_sources()
                ...
                with st.spinner(f"Fetching and ingesting {url_input}..."):
                    try:
                        ...
                        if chunks > 0:
                            ...
                            st.info("URL already ingested (0 new chunks)")
                        st.session_state.ingested = list_ingested_sources()
                    except Exception as e:
                        st.error(f"Failed: {e}")
            else:
    ...
    if ingested:
        for fname in ingested:
            ...
            if col2.button("Remove", key=f"del_{fname}", help=f"Remove {fname}"):
    ...
    if ingested:
        ...
        st.multiselect(
            ...
            options=ingested,
            key="doc_filter",
            help="Empty means search all documents. Pick one or more to scope queries.",
        )
    ...
    if installed:
        ...
        st.warning(
            ...
            "Run `make pull-light` or `ollama pull <model>`."
        )
    ...
    st.toggle(
        ...
        key="use_rewrite",
        help="Uses the LLM to turn 'what about chapter 4?' into a standalone query "
             "before retrieval. Adds one extra LLM call per turn.",
    )
    ...
    with st.expander("Advanced settings"):
        st.slider(
            ...
            key="adv_temperature",
            help="0 is deterministic, 1 is creative. RAG usually wants low temperature.",
        )
        st.number_input(
        ...
        st.text_area(
            ...
            help="Overrides the default citation rules. Leave the default "
    ...
    if st.button("Clear conversation", use_container_width=True, disabled=not has_msgs):
        st.session_state.messages = []
        st.rerun()
                ...
                try:
                    for event in stream_query(
                        ...
                        if event["type"] == "sources":
                            ...
                            answer_placeholder.markdown(accumulated + "_")
